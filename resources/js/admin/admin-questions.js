...
function toggleAllQuestions(category) {
    ...
    const toggleButtons = Array.from(questionCards).map(card => card.querySelector('.btn-toggle'));
    
    // Check if all are expanded or collapsed
    const expandedCount = toggleButtons.filter(btn => btn.classList.contains('expanded')).length;
    const shouldExpand = expandedCount < toggleButtons.length / 2;
    
    toggleButtons.forEach(button => {
        const questionDetails = button.closest('.question-card').querySelector('.question-details');
    ...
    // Update button text
    const toggleAllBtn = categoryContainer.parentElement.querySelector('.section-actions .btn-outline');
    if (toggleAllBtn) {
...
function refreshApiQuestions() {
    ...
    const refreshBtn = document.querySelector('[onclick="refreshApiQuestions()"]');
    ...
    loadingDiv.innerHTML = '<div class="spinner"></div> Fetching fresh API questions...';
...
function initializeKeyboardShortcuts() {
    document.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'Escape') {
            const allToggleButtons = document.querySelectorAll('.btn-toggle.expanded');
...
function searchQuestions(searchTerm) {
    ...
    questionCards.forEach(card => {
        ...
        const isMatch = questionText.includes(normalizedSearch) || 
