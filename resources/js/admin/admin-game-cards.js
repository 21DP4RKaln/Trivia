...
class GameCardManager {
  ...
  populateModal(game) {
    ...
    if (avatarText)
      avatarText.textContent = (game.user?.name || 'G').charAt(0).toUpperCase();
    ...
    if (accuracyAvg)
      accuracyAvg.textContent = `${(game.user?.average_accuracy || game.accuracy).toFixed(1)}%`;
    ...
    if (scorePercentage)
      scorePercentage.textContent = `${((game.correct_answers / 20) * 100).toFixed(1)}%`;
    if (accuracyValue) accuracyValue.textContent = game.accuracy.toFixed(1);
    if (accuracyFill) accuracyFill.style.width = `${game.accuracy}%`;
    if (durationValue)
      durationValue.textContent = this.formatDuration(game.duration_seconds);
    if (avgQuestionTime)
      avgQuestionTime.textContent = `${(game.duration_seconds / 20).toFixed(1)}s`;
    ...
    if (relativeTime)
      relativeTime.textContent = this.getRelativeTime(game.created_at);
